...
- `currency-exchange` mikroszolgáltatás `OTEL_*` környezeti változókkal
...
kubectl get svc -n pay-api-prod
```
...
- a `pay-api` kliens oldali HTTP span a `currency-exchange` felé
- a `currency-exchange` szerver oldali HTTP span
- a span attribútumai:
  - összeg
  - pénznem
  - részletszám
...
Mivel a `pay-api` már most is egy külön mikroszolgáltatást hív meg, ugyanabban a trace-ben meg fog jelenni a szolgáltatások közötti lánc.
